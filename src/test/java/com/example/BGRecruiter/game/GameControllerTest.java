...
class GameControllerTest {
    /**
     * Method under test: {@link GameController#getGame()}
     */
    @Test
    void testGetGame() {
        ...
        List<Game> actualGame = (new GameController(
                new GameService(gameRepository, new GamerService(mock(GamerRepository.class), mock(GameRepository.class)))))
                .getGame();
    ...
    void testGetGame2() {
        ...
        ArrayList<Game> gameList = new ArrayList<>();
        when(gameService.getGame()).thenReturn(gameList);
        List<Game> actualGame = (new GameController(gameService)).getGame();
        assertSame(gameList, actualGame);
        assertTrue(actualGame.isEmpty());
        verify(gameService).getGame();
    }


    /**
     * Method under test: {@link GameController#getRecommendations(String)}
     */
    @Test
    void testGetRecommendations() {
        ...
        Optional<List<Game>> actualRecommendations = (new GameController(
                new GameService(gameRepository, new GamerService(gamerRepository, mock(GameRepository.class)))))
                .getRecommendations("jane.doe@example.org");
